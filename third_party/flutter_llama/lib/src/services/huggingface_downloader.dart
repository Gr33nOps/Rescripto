...
class HuggingFaceDownloader {
  ...
  Future<String> downloadFile({
    ...
    try {
      ...
      final modelsDir =
          Directory(path.join(appDir.path, 'models', 'huggingface'));
          ...
          if (contentLength > 0) {
            ...
            if (kDebugMode &&
                (receivedBytes ~/ (10 * 1024 * 1024)) >
                    ((receivedBytes - chunk.length) ~/ (10 * 1024 * 1024))) {
              final mb = receivedBytes / 1024 / 1024;
              final totalMb = contentLength / 1024 / 1024;
              debugPrint(
                '[HuggingFaceDownloader] Downloaded: '
      ...
      if (contentLength > 0 && fileSize != contentLength) {
        await file.delete();
        throw Exception(
            'Download incomplete: expected $contentLength bytes, got $fileSize');
      }
      ...
      if (kDebugMode) {
        ...
        print(
            '[HuggingFaceDownloader] File size: ${(fileSize / 1024 / 1024).toStringAsFixed(2)} MB');
      }
      ...
      throw ModelDownloadException(
          modelId, 'Failed to download from HuggingFace', e);
    }
  }
  ...
  Future<String> downloadGGUFModel({
    ...
    try {
      ...
      if (kDebugMode) {
        print(
            '[HuggingFaceDownloader] Found GGUF file: ${firstGGUF.name} (${firstGGUF.sizeFormatted})');
      }
