...
class OllamaDownloader {
  ...
  Future<String> exportModelToGGUF({
    ...
    try {
      ...
      final fileName =
          '${modelName.replaceAll('/', '_').replaceAll(':', '_')}.gguf';
      ...
      if (whichResult.exitCode != 0) {
        throw Exception(
            'Ollama CLI not found. Please install from https://ollama.com');
      }
  ...
  Future<String?> getModelPath(String modelName) async {
    try {
      ...
      if (model.digest != null) {
        final blobPath = path.join(
            ollamaDir, model.digest!.replaceFirst('sha256:', 'sha256-'));
