...
class WhisperDownloader {
  ...
  Future<String> download(
    ...
    if (await _isValid(file, model, verifyHash: config.verifyIntegrity)) {
      return file.path;
    }
    ...
    while (true) {
      ...
      try {
        ...
        if (!await _isValid(
          part,
          model,
          verifyHash: config.verifyIntegrity,
        )) {
          final actual = await part.length();
          await part.delete();
          throw WhisperError(
            'Download corrupt ($actual bytes, expected '
            '${model.fileSizeBytes} with the published checksum)',
  ...
  Future<bool> _isValid(
    ...
    if (!await file.exists() || await file.length() != model.fileSizeBytes) {
      return false;
    }
