...
void main() {
  ...
  setUp(() async {
    ...
    providerRegistry = ProviderRegistry(
      ProviderStore(
        database,
        CredentialStore(database, storage: FakeSecureStorage()),
      ),
    );
  ...
  Future<ProviderConfig> configureProviderWithoutSelecting(
    String presetId,
  ) async {
  ...
  group('TargetRouter — cloud selection is derived, not required', () {
    ...
    test(
      'routes to an enabled provider even with nothing explicitly selected',
      () async {
    ...
    test(
      'a model ref left over from another provider is not sent to this one',
      () async {
        await settings.setProcessingMode(ProcessingMode.cloud);
        ...
        expect(
          ...
          reason:
              'sending another provider\'s model name would 404 confusingly',
        );
      },
    );
  ...
  group('TargetRouter — Cloud mode', () {
    test(
      'blocks with noCloudProvider when policy allows cloud but nothing is configured',
      () async {
        await settings.setProcessingMode(ProcessingMode.cloud);
        await networkPolicy.setFeatureEnabled(
    ...
    test(
      'blocks with cloudDisabledByPolicy when nothing is configured and policy is off',
      () async {
        await settings.setProcessingMode(ProcessingMode.cloud);
    ...
    test(
      'blocks with cloudDisabledByPolicy when a provider exists but policy forbids it',
      () async {
        await settings.setProcessingMode(ProcessingMode.cloud);
    ...
    test(
      'the kill switch blocks cloud mode even with a configured provider',
      () async {
        await settings.setProcessingMode(ProcessingMode.cloud);
        await networkPolicy.setFeatureEnabled(
          NetworkFeature.cloudRewrite,
  ...
  group('TargetRouter — Hybrid mode', () {
    test('short input prefers local when both are available', () async {
      ...
      expect(decision.reason, contains('on-device'));
    });
    ...
    test('long input prefers cloud when both are available', () async {
      ...
      final decision = router().route(
        inputLength: TargetRouter.hybridLengthThreshold + 1,
      );
      expect(decision.target!.engineId, 'cloud.openaiCompatible');
      expect(decision.fallback, isNotNull);
    ...
    test(
      'falls back to local for long input when cloud is not configured',
      () async {
        await settings.setProcessingMode(ProcessingMode.hybrid);
        final decision = router().route(
          inputLength: TargetRouter.hybridLengthThreshold + 1,
    ...
    test(
      'falls back to cloud when local is not installed, regardless of length',
      () async {
        await settings.setProcessingMode(ProcessingMode.hybrid);
        await networkPolicy.setFeatureEnabled(
          NetworkFeature.cloudRewrite,
    ...
    test(
      'every decision names which side and why in the reason string',
      () async {
        await settings.setProcessingMode(ProcessingMode.hybrid);
        final decision = router().route(inputLength: 10);
