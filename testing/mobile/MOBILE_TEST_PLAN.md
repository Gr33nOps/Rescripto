...
Read this before running a QA pass. It is written so that anyone can pick up
the work without relying on context from an earlier session.
...
4. `testing\mobile\build_and_install.ps1`: build the current debug APK,
   ...
   your last pass.
5. Confirm your Appium MCP client is connected and start a session using
   `testing/mobile/capabilities.json`.
...
6. Persistence: change a setting, force-stop the app
   ...
   so for a persistence check use `adb shell am force-stop com.rescripto.rescripto`
...
11. Screenshot liberally throughout, in `testing/mobile/screenshots/`,
...
- **Cloud**: requires a configured, enabled provider with a key (Settings →
  ...
  API, so don't run more than the minimum needed to confirm the plumbing
...
- **Quick Settings tile**: add the Rescripto tile to Quick Settings
  (`adb shell` can't easily trigger this, so do it manually via the emulator
...
- **Clipboard**: Paste icon in the source field reads real clipboard
  content (`adb shell am broadcast` can't set clipboard directly, so set it
...
Current baseline: `flutter test` passed 372 tests at v1.2.1. Re-run the suite
for your own change and report the result you actually observe.
